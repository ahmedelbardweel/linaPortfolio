            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="bg-[#f0f0ef] dark:bg-[#2a2a28] border-b border-[#e3e3e0] dark:border-[#3E3E3A]">
                        ...
                        <th class="text-left px-4 py-3 font-medium text-[#706f6c]">Thumbnail</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse ($reels as $reel)
                        <tr class="hover:bg-[#f0f0ef] dark:bg-[#2a2a28]">
                            ...
                            <td class="px-4 py-3 font-medium text-[#1b1b18] dark:text-[#EDEDEC]">{{ $reel->title }}</td>
                            <td class="px-4 py-3 text-[#706f6c]">{{ $reel->order }}</td>
                            <td class="px-4 py-3">
                                <form method="POST" action="{{ route('admin.reels.toggle', $reel) }}">
                                    @csrf
                                    @method('PATCH')
                                    <button type="submit" class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium {{ $reel->is_active ? 'bg-green-100 text-green-700' : 'bg-[#e3e3e0] dark:bg-[#3E3E3A] text-[#706f6c]' }}">
                            ...
                            <td class="px-4 py-3 text-right">
                                <a href="{{ route('admin.reels.edit', $reel) }}" class="inline-flex items-center gap-1 text-sm text-[#c42802] hover:text-[#f53003] mr-3">
