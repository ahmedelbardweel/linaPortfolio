...
@section('content')
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-bold text-[#1b1b18] dark:text-[#EDEDEC]" data-translate-key="Stories">{{ __("Stories") }}</h1>
        <a href="{{ route('admin.stories.create') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-[#c42802] hover:bg-[#f53003] text-white text-sm font-medium rounded-lg transition-colors" data-translate-key="Add New">
    ...
    <div class="grid gap-3" style="grid-template-columns:repeat(auto-fill,minmax(160px,1fr))">
                    ...
                    <div class="flex items-center justify-between mt-2 pt-2 border-t border-[#e3e3e0] dark:border-[#3E3E3A]">
                        <form method="POST" action="{{ route('admin.stories.toggle', $story) }}">
                            ...
                            <button type="submit" class="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[10px] font-medium {{ $story->is_active ? 'bg-green-100 text-green-700' : 'bg-[#e3e3e0] dark:bg-[#3E3E3A] text-[#706f6c]' }}">
                                ...
                                <span data-translate-key="{{ $story->is_active ? 'Active' : 'Inactive' }}">{{ $story->is_active ? __("Active") : __("Inactive") }}</span>
                            </button>
                        </form>
                        <div class="flex items-center gap-1">
                            <a href="{{ route('admin.stories.edit', $story) }}" class="inline-flex items-center justify-center w-7 h-7 rounded text-[#c42802] hover:text-[#f53003] hover:bg-[#f0f0ef] dark:hover:bg-[#2a2a28] transition-all" title="{{ __("Edit") }}" data-translate-key="Edit">
                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                            </a>
                            <form method="POST" action="{{ route('admin.stories.destroy', $story) }}" class="inline" onsubmit="return confirm('{{ __("Are you sure?") }}')">
                                @csrf
                                @method('DELETE')
        ...
        @empty
            <div class="col-span-full text-center py-12 text-[#706f6c]">
                ...
                <p class="text-sm" data-translate-key="No stories yet.">{{ __("No stories yet.") }}</p>
                <a href="{{ route('admin.stories.create') }}" class="mt-2 inline-block text-sm text-[#c42802] hover:text-[#f53003]" data-translate-key="Add your first story">{{ __("Add your first story") }}</a>
            </div>
        @endforelse
    </div>
@endsection
